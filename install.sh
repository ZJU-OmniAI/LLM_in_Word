...
if [ -z "$NODE_BIN" ]; then
  echo "❌ 未找到 node。请先安装 Node.js ≥ 18（本机 nvm 用户请先 source ~/.zshrc）"
  exit 1
fi
...
cp -R "$PROJ/server" "$PROJ/taskpane" "$PROJ/assets" "$PROJ/package.json" "$STAGE/"
...
if $UPDATE_ONLY; then
...
elif security find-certificate -c "word_edit-localhost" "$HOME/Library/Keychains/login.keychain-db" >/dev/null 2>&1; then
  ...
  if ! security add-trusted-cert -r trustRoot -k "$HOME/Library/Keychains/login.keychain-db" "$CRT"; then
    ...
    echo "      然后在「钥匙串访问」里找到 word_edit-localhost，双击 → 信任 → 始终信任"
  fi
fi
...
# 用 printf %q 生成安全的 shell 字面量，不 eval 代理值，也不把凭证打印到日志。
{
  ...
  if [ -n "$CLAUDE_PATH" ]; then printf 'export WORD_EDIT_CLAUDE_BIN=%q\n' "$CLAUDE_PATH"; fi
  if [ -n "$CODEX_PATH" ]; then printf 'export WORD_EDIT_CODEX_BIN=%q\n' "$CODEX_PATH"; fi
  printf 'export WORD_EDIT_PORT=%q\n' "$PORT"
...
mkdir -p "$WEF"
cp "$PROJ/manifest.xml" "$WEF/word_edit-manifest.xml"

# ---- 6) 健康检查（必须绕开代理：curl 走 http_proxy 的话本机回环会被代理吞掉而误报）----
ok=""
for i in 1 2 3 4 5 6 7 8; do
  sleep 1
  if curl -sk --noproxy '*' --max-time 3 "https://127.0.0.1:$PORT/api/ping" | /usr/bin/grep -q '"ok":true'; then
    ok=1; break
  fi
done
if [ -n "$ok" ]; then
  echo ""
  echo "✅ 安装完成，服务已在 https://localhost:$PORT 运行。接下来："
else
  echo ""
  echo "⚠️ 服务好像没起来，看日志：tail -20 $BASE/server.log"
  echo "   （常见原因：端口被占 / node 路径变了。修好后重启：launchctl kickstart -k gui/\$(id -u)/${LABEL}）"
  exit 1
fi
if $UPDATE_ONLY; then
  echo "  更新已完成，在 Word 的 AI 改写面板点右上角 ⟳ 即可加载新版。"
  exit 0
fi
echo "  1. 完全退出 Word（Cmd+Q）再重新打开"
echo "  2. 功能区「插入」→「加载项」（或「我的加载项」下拉小箭头）→「开发人员」区域 → 选「AI 改写」"
echo "     （首次打开如果面板空白：多半是证书信任没生效，重跑本脚本或看 README 排查）"
echo "  3. 以后每次打开 Word，「开始」功能区右侧会有「✦ AI 改写」按钮，点击开合面板"
echo ""
echo "卸载：launchctl bootout gui/\$(id -u)/${LABEL} && rm -rf ~/.word_edit $PLIST $WEF/word_edit-manifest.xml"
