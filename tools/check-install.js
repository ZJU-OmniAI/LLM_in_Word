...
for (let i = 0; i < 15; i++) {
  const ok = await new Promise(resolve => {
    const req = https.get('https://127.0.0.1:8377/api/ping', { ca: trust, timeout: 2000 }, res => {
      ...
      res.on('end', () => { try { resolve(res.statusCode === 200 && JSON.parse(data).version === '0.6.0'); } catch { resolve(false); } });
    });
    req.on('timeout', () => req.destroy()); req.on('error', () => resolve(false));
  });
  if (ok) { console.log('HTTPS certificate and LLM_in_Word 0.6.0 health check passed.'); process.exit(0); }
